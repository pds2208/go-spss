...
readstat_variable_t *save_header(SavHeader *const *sav_header, int column_cnt,
...
int save_sav(const char *output_file, const char *label, SavHeader **sav_header, int column_cnt,
             int data_rows, SavData **sav_data) {
    ...
    for (int j = 0; j < column_cnt; j++) {
      ...
      switch (sav_data[i][j].sav_type) {
        case READSTAT_TYPE_STRING:
          readstat_insert_string_value(writer, variable, (const char *)sav_data[i][j].string_value);
          break;

        case READSTAT_TYPE_INT8:
          readstat_insert_int8_value(writer, variable, sav_data[i][j].int_value);
          break;

        case READSTAT_TYPE_INT16:
          readstat_insert_int16_value(writer, variable, sav_data[i][j].int_value);
          break;

        case READSTAT_TYPE_INT32:
          readstat_insert_int32_value(writer, variable, sav_data[i][j].int_value);
          break;

        case READSTAT_TYPE_FLOAT:
          readstat_insert_float_value(writer, variable, sav_data[i][j].float_value);
          break;

        case READSTAT_TYPE_DOUBLE:
          readstat_insert_double_value(writer, variable, sav_data[i][j].double_value);
          break;
